    ...
    while True:
        ...
        if key == ord("k"):
            ...
            tiles_dict, warped = warp_and_segment_chessboard(
                frame, pts, output_dir="debug/tiles"
            )
            ...
            save_tiles_by_coords(
                tiles_list, calibration_mapping, output_dir="debug/chess_squares"
            )
            ...
            save_game_state(
                curr_simple, move_number, "simple", dir_path=GAME_STATES_DIR
            )

            move_full, piece_code = detect_move_full(
                prev_simple, curr_simple, prev_full
            )
            if move_full:
                print(f"(Full) ruch: {move_full} ({piece_code})")
                with open(moves_path, "a", encoding="utf-8") as f:
                    f.write(f"{move_number}: {move_full} ({piece_code})\n")
